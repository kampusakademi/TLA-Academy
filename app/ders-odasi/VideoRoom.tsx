...
import {
  Mic, MicOff, Video, VideoOff, PhoneOff, Share2, MessageSquare, Unlock, Lock, X, Send, User, Users, PenTool, Trash2, Eraser, Type, Search, MousePointer2, MonitorUp
} from 'lucide-react';
...
function RoomContent({ channelName, userRole }: { channelName: string, userRole: string }) {
  ...
  const logUserAction = async (action: 'Odaya Girdi' | 'Odadan Çıktı') => {
    ...
    try {
      await supabase.from('ders_loglari').insert([{
        ders_id: channelName,
        kullanici_rolu: benimRolum,
        aksiyon: action,
    ...
    roomChannel
      .on('broadcast', { event: 'room_action' }, (payload) => {
        ...
        if (veri.type === 'chat') {
          ...
          veri.lines.forEach((line: any) => { linesRef.current.push(line); });
          redrawAll();
        }
          ...
          veri.objects.forEach((newObj: CanvasObject) => {
            if (newObj.type === 'image') {
              ...
              if (existing && existing.url === newObj.url && existing.imgElement) {
                ...
                const img = new Image(); img.crossOrigin = 'Anonymous'; img.src = newObj.url!; newObj.imgElement = img; img.onload = redrawAll;
              }
            }
          });
          ...
          linesRef.current = []; objectsRef.current = []; redrawAll();
        }
      })
      .subscribe();
  ...
  const getPos = (e: any) => {
    ...
    return { x: (clientX - rect.left) / rect.width, y: (clientY - rect.top) / rect.height, clientX: clientX - rect.left, clientY: clientY - rect.top };
  };

  const emitObjects = () => {
    supabase.channel(`ders_odasi_${channelName}`).send({ type: 'broadcast', event: 'room_action', payload: { type: 'sync_objects', objects: objectsRef.current.map(o => ({...o, imgElement: undefined})) } });
  };
  ...
  const redrawAll = () => {
    ...
    linesRef.current.forEach(line => {
      ...
      ctx.strokeStyle = line.color; ctx.lineWidth = line.width; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
      ctx.stroke(); ctx.closePath();
    });

    objectsRef.current.forEach(obj => {
      if (obj.type === 'text') {
        ctx.font = 'bold 24px Inter, sans-serif'; ctx.fillStyle = obj.color || '#000'; ctx.textBaseline = "top";
        ctx.fillText(obj.text || '', obj.x * canvas.width, obj.y * canvas.height);
        const metrics = ctx.measureText(obj.text || '');
        obj.w = metrics.width / canvas.width; obj.h = 30 / canvas.height;
      ...
      if (activeTool === 'move' && selectedObjIdRef.current === obj.id) {
        ...
        ctx.setLineDash([]);
        if (obj.type === 'image') { ctx.fillStyle = '#3b82f6'; ctx.fillRect((obj.x + obj.w) * canvas.width - 6, (obj.y + obj.h) * canvas.height - 6, 12, 12); }
      }
    });
  };
  ...
  useEffect(() => {
    const resizeCanvas = () => {
      const canvas = canvasRef.current;
      if (canvas && canvas.parentElement) { canvas.width = canvas.parentElement.clientWidth; canvas.height = canvas.parentElement.clientHeight; redrawAll(); }
    };
    if (isWhiteboardActive) { setTimeout(resizeCanvas, 100); window.addEventListener('resize', resizeCanvas); return () => window.removeEventListener('resize', resizeCanvas); }
  }, [isWhiteboardActive]);
  ...
  const getHitResult = (nx: number, ny: number) => {
    ...
    for (let i = objectsRef.current.length - 1; i >= 0; i--) {
      ...
      if (obj.type === 'image' && selectedObjIdRef.current === obj.id) {
        ...
        if (Math.abs(nx - (obj.x + obj.w)) < handleR && Math.abs(ny - (obj.y + obj.h)) < handleR) return { action: 'resize', id: obj.id };
      }
      if (nx >= obj.x && nx <= obj.x + obj.w && ny >= obj.y && ny <= obj.y + obj.h) return { action: 'drag', id: obj.id };
    }
    return null;
  };
  ...
  const startDraw = (e: any) => {
    ...
    if (activeTool === 'text') { setTextInput({ visible: true, id: Date.now().toString(), x: pos.clientX, y: pos.clientY, realX: pos.x, realY: pos.y, text: '' }); return; }
    if (activeTool === 'move') {
      const hit = getHitResult(pos.x, pos.y);
      if (hit) { selectedObjIdRef.current = hit.id; const obj = objectsRef.current.find(o => o.id === hit.id); interactRef.current = { action: hit.action, id: hit.id, offsetX: pos.x - (obj?.x || 0), offsetY: pos.y - (obj?.y || 0) }; } 
      else { selectedObjIdRef.current = null; }
      redrawAll(); return;
    }
    isDrawingRef.current = true; posRef.current = pos;
  };
  ...
  const moveDraw = (e: any) => {
    ...
    if (activeTool === 'move' && interactRef.current.action !== 'none') {
      ...
      if (obj) {
        if (interactRef.current.action === 'drag') { obj.x = pos.x - interactRef.current.offsetX; obj.y = pos.y - interactRef.current.offsetY; } 
        else if (interactRef.current.action === 'resize') { obj.w = Math.max(0.05, pos.x - obj.x); obj.h = Math.max(0.05, pos.y - obj.y); }
        redrawAll();
        const now = Date.now(); if (now - lastEmitTime.current > 40) { emitObjects(); lastEmitTime.current = now; }
      }
      return;
    }
    ...
    const now = Date.now();
    if (now - lastEmitTime.current > 40) { supabase.channel(`ders_odasi_${channelName}`).send({ type: 'broadcast', event: 'room_action', payload: { type: 'draw_batch', lines: pendingDraws.current } }); pendingDraws.current = []; lastEmitTime.current = now; }
    posRef.current = pos; redrawAll();
  };

  const stopDraw = () => {
    if (activeTool === 'move') { interactRef.current = { action: 'none', id: '', offsetX: 0, offsetY: 0 }; emitObjects(); }
    if (isDrawingRef.current && pendingDraws.current.length > 0) { supabase.channel(`ders_odasi_${channelName}`).send({ type: 'broadcast', event: 'room_action', payload: { type: 'draw_batch', lines: pendingDraws.current } }); pendingDraws.current = []; }
    isDrawingRef.current = false;
  };

  const clearCanvas = (emit: boolean) => {
    linesRef.current = []; objectsRef.current = []; selectedObjIdRef.current = null; redrawAll();
    if (emit) supabase.channel(`ders_odasi_${channelName}`).send({ type: 'broadcast', event: 'room_action', payload: { type: 'clear_board' } });
  };

  const searchImages = async (e: React.FormEvent) => {
    e.preventDefault(); if (!imageSearchQuery.trim()) return; setIsSearchingImage(true); setImageSearchResults([]);
    try {
      const transRes = await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(imageSearchQuery.trim())}&langpair=tr|en`);
      const transData = await transRes.json();
      let englishQuery = transData?.responseData?.translatedText || imageSearchQuery; 
  ...
  const addImageToCanvas = (url: string) => {
    const id = Date.now().toString(); const img = new Image(); img.crossOrigin = "Anonymous";
    img.onload = () => {
      const canvas = canvasRef.current; if (!canvas) return;
      const w = 0.3; const h = w * (canvas.width / canvas.height) / (img.width / img.height);
      objectsRef.current.push({ id, type: 'image', url, x: 0.35, y: 0.35, w, h, imgElement: img });
      selectedObjIdRef.current = id; setActiveTool('move'); redrawAll(); emitObjects();
    };
  ...
  useEffect(() => { audioTracks.forEach(track => { try { track.play(); } catch(e) { } }); }, [audioTracks]);
    ...
    <div style={{ height: '100vh', width: '100vw', display: 'flex', flexDirection: 'column', backgroundColor: '#f1f5f9', color: '#0f172a', fontFamily: '"Inter", system-ui, sans-serif', overflow: 'hidden' }}>
      ...
      <footer style={{ paddingBottom: '24px', display: 'flex', justifyContent: 'center', zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', backgroundColor: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(16px)', padding: '10px 20px', borderRadius: '40px', border: '1px solid rgba(226, 232, 240, 0.8)', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.08)' }}>
          ...
          {gercekRol === "ogretmen" ? (
            <>
              <button 
                ...
                style={{ ...controlBtn(isWhiteboardActive), width: 'auto', borderRadius: '24px', padding: '0 20px', gap: '8px', fontWeight: 700, fontSize: '0.9rem', backgroundColor: isWhiteboardActive ? '#eef2ff' : '#ffffff', color: isWhiteboardActive ? '#4f46e5' : '#0f172a', borderColor: isWhiteboardActive ? '#c7d2fe' : '#e2e8f0' }}
              ...
              <button 
                ...
                style={{
                  ...
                  width: 'auto', borderRadius: '24px', padding: '0 20px', gap: '8px', fontWeight: 800, fontSize: '0.9rem', cursor: isScreenStarting ? 'wait' : 'pointer',
              ...
              >
                <MonitorUp size={18} />
                <span>{isScreenStarting ? 'Hazırlanıyor...' : (isLocalSharing ? 'Paylaşımı Durdur' : 'Ekranı Paylaş')}</span>
              </button>
            
              <button 
                onClick={toggleStudentPermission} 
                title="Öğrenci Ekran İznini Değiştir" 
                style={{ backgroundColor: teacherPermissionState ? '#dcfce7' : '#f1f5f9', color: teacherPermissionState ? '#16a34a' : '#475569', border: '1px solid', borderColor: teacherPermissionState ? '#bbf7d0' : '#e2e8f0', height: '48px', padding: '0 20px', borderRadius: '24px', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 2px 4px rgba(0,0,0,0.03)' }}
                onMouseEnter={e => e.currentTarget.style.filter = 'brightness(0.95)'} onMouseLeave={e => e.currentTarget.style.filter = 'none'}
              >
            ...
            <button 
              ...
              style={{ ...controlBtn(isLocalSharing), width: 'auto', borderRadius: '24px', padding: '0 20px', gap: '8px', fontWeight: 700, fontSize: '0.9rem', opacity: (!studentScreenAllowed && !isLocalSharing) ? 0.5 : 1, cursor: isScreenStarting ? 'wait' : ((!studentScreenAllowed && !isLocalSharing) ? 'not-allowed' : 'pointer'), backgroundColor: isScreenStarting ? '#eab308' : (isLocalSharing ? '#fee2e2' : '#ffffff'), color: isScreenStarting ? '#ffffff' : (isLocalSharing ? '#ef4444' : '#0f172a'), borderColor: isScreenStarting ? '#eab308' : (isLocalSharing ? '#fca5a5' : '#e2e8f0') }}
              title={(!studentScreenAllowed && !isLocalSharing) ? "Öğretmeninizden izin istemelisiniz" : ""}
              onMouseEnter={e => { if(studentScreenAllowed || isLocalSharing) e.currentTarget.style.filter = 'brightness(0.95)' }} onMouseLeave={e => { e.currentTarget.style.filter = 'none' }}
            >
              <MonitorUp size={18} />
              <span>{isScreenStarting ? 'Hazırlanıyor...' : (isLocalSharing ? 'Paylaşımı Durdur' : 'Ekranı Paylaş')}</span>
          ...
          <button 
            onClick={leaveRoom} 
            style={{ backgroundColor: '#ef4444', color: 'white', border: 'none', height: '48px', padding: '0 24px', borderRadius: '24px', fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 12px rgba(239, 68, 68, 0.25)' }}
          ...
          <div style={{ background: '#ffffff', width: '100%', maxWidth: '700px', borderRadius: '24px', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
            ...
            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px', flex: 1, overflowY: 'auto' }}>
              <form onSubmit={searchImages} style={{ display: 'flex', gap: 12 }}>
                <input 
                  autoFocus value={imageSearchQuery} onChange={(e) => setImageSearchQuery(e.target.value)} placeholder="Örn: Araba, Kedi, İstanbul..." 
              ...
              {imageSearchResults.length > 0 ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                  {imageSearchResults.map((url, i) => (
                    <div 
                      key={i} onClick={() => addImageToCanvas(url)}
                      ...
                      onMouseEnter={e => e.currentTarget.style.borderColor = '#4f46e5'} onMouseLeave={e => e.currentTarget.style.borderColor = 'transparent'}
                    />
                  ))}
                </div>
              ) : (
...
function controlBtn(isActive: boolean): React.CSSProperties {
  return { width: '48px', height: '48px', borderRadius: '50%', border: '1px solid', borderColor: isActive ? '#fca5a5' : '#e2e8f0', backgroundColor: isActive ? '#ef4444' : '#ffffff', color: isActive ? '#ffffff' : '#0f172a', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', boxShadow: '0 2px 4px rgba(0,0,0,0.03)' };
}
...
function NameBadge({ name, isLocal }: { name: string, isLocal: boolean }) {
  return (
    <div style={{ position: 'absolute', bottom: '16px', left: '16px', backgroundColor: 'rgba(15, 23, 42, 0.75)', backdropFilter: 'blur(8px)', padding: '6px 14px', borderRadius: '20px', fontSize: '0.85rem', fontWeight: 600, color: '#ffffff', display: 'flex', alignItems: 'center', gap: '6px', zIndex: 10 }}>
      {isLocal ? <User size={14} color="#10b981" /> : <Users size={14} color="#3b82f6" />}<span>{name}</span>
    </div>
  );
}
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#0f172a', color: '#64748b' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
        <div style={{ width: '64px', height: '64px', borderRadius: '50%', backgroundColor: '#1e293b', color: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #334155' }}><VideoOff size={28} /></div>
